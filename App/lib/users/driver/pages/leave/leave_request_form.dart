...
class _LeaveRequestFormPageState extends State<LeaveRequestFormPage> {
  ...
  void dispose() {
    ...
    // Reset provider state so the next form open is clean (cannot use [context] here).
    ...
    switch (_step) {
      ...
      case _LeaveRequestStep.review:
        return _ReviewRequestView(
          ...
          onEdit: () {
            setState(() => _step = _LeaveRequestStep.form);
          },
  ...
  // Called when driver taps "Continue" on the form step.
  // Runs the conflict check, then either shows the warning dialog
  // (conflict found) or goes straight to review (no conflict).
  ...
  // Called when driver taps "Submit Request" on the review step.
  Future<void> _handleSubmit() async {
    ...
    await provider.submitRequest(
      ...
      // Attachment upload to Supabase Storage is out of scope here.
      // Pass null until storage is wired up.
      attachmentUrl: null,
    );
  ...
  /// Clear, calm explanation when dates overlap an existing pending/approved request.
  ...
  // Shows the conflict warning dialog listing affected job names.
  // Driver can proceed anyway or go back to edit dates.
...
class _LeaveRequestFormView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Show a loading overlay on the Continue button while conflict check runs
