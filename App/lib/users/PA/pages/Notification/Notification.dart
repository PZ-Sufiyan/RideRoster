...
enum _NotificationFilter { all, leaveStatus, message, newJobs, document, unread }

class PaNotificationsPage extends StatefulWidget {
  const PaNotificationsPage({super.key});

  @override
  State<PaNotificationsPage> createState() =>
      _PaNotificationsPageState();
}
...
class _PaNotificationsPageState extends State<PaNotificationsPage> {
  ...
  List<UserNotificationItem> get _filteredItems {
  ...
  void _toggleSelect(UserNotificationItem item) {
    setState(() {
      _selectionMode = true;
      if (_selectedIds.contains(item.id)) {
        _selectedIds.remove(item.id);
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected
              ? _PaNotificationColors.iconBlueBg.withValues(alpha: 0.35)
              : AppColors.background,
          borderRadius: BorderRadius.circular(SizeConfig.radiusLG),
          border: selected
              ? Border.all(color: _PaNotificationColors.primary, width: 1.5)
              : null,
          boxShadow: [
            BoxShadow(
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (filter == _NotificationFilter.unread) {
      ...
      message = 'No new job assignments yet.';
