...
import { useLeaveRequestsList } from '../hooks/useLeaveRequestsList'
...
function AffectedRoutesSummary({ contexts, jobsBasePath }) {
...
function RequestRow({ r, notesDraft, setNoteFor, busyId, onDecision, jobsBasePath }) {
    ...
    <div className="border-b border-gray-100 px-5 py-5 last:border-b-0">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-5">
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <AffectedRoutesSummary contexts={r.jobContexts} jobsBasePath={jobsBasePath} />
...
/**
 * @param {{ jobsBasePath?: string, title?: string }} props
 */
export default function LeaveRequestsPage({
  jobsBasePath = '/portal/jobs',
  title = 'Leave requests',
}) {
  const { rows, loading, error: loadError, reload } = useLeaveRequestsList()
  ...
  const handleDecision = async (requestId, status) => {
    ...
    try {
      ...
      setActionError(e?.message || 'Could not update request.')
    } finally {
      setBusyId(null)
    }
  }
