...
/** Visual stack only — used by ToastProvider (no context import). */
export function ToastStackView({ toasts = [], onClose, placement = 'top-right' }) {
    if (!toasts.length) return null;

    const positionClass = PLACEMENT_CLASS[placement] || PLACEMENT_CLASS['top-right'];

    return (
        <div className={`fixed ${positionClass} z-[9999] w-[min(420px,calc(100vw-2rem))] space-y-2`}>
            {toasts.map((toast) => (
                <Toast
                    key={toast.id}
                    type={toast.type}
